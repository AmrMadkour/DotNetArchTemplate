...
Do not delete or "clean up" work-in-progress/incomplete code unless asked — half-finished pieces (e.g. `Error.cs`/`OrderValidationException`, unused since `Result` moved to a plain string message) may be intentionally parked mid-refactor. If something looks broken, ask before fixing it, unless you were already told to leave it alone.
...
- **`Tests/ApplicationTests/Results/ResultTests.cs`** has one failing case: it expects `Result<TValue>.Failure(null)` to throw `ArgumentException`, but `ArgumentException.ThrowIfNullOrEmpty` throws the more specific `ArgumentNullException` for a null input. Parked, not fixed yet (1 failing test out of 32).
