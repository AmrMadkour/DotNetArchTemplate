...
- **`Domain/`** — `Models/Order`, `Item`, `Quote`, `Coupon`, `DiscountType`; `Services/IDiscountStrategy` + its two implementations (`PercentageDiscountStrategy`, `FlatAmountDiscountStrategy`) and `DiscountStrategyFactory` (parked Strategy/Factory scaffold, not wired into `QuoteService` yet). Zero dependencies on any other project or framework. `Order`/`Item` now own domain *behavior*: `string? Validate()` (invariant checks, per Rule 9) and `CalculateSubtotal()` (computation) live on the entities themselves, not in `Application`.
