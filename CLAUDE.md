...
15. Picking a request-tracing approach: use the framework's own per-request ID (e.g. `HttpContext.TraceIdentifier`) for single-request log correlation with zero setup; mint a separate CorrelationId (forwarded as a header) when one logical operation spans multiple hops/services and needs to be greppable end-to-end; reach for OpenTelemetry only once there's a real multi-service/multi-language system with an actual tracing backend to send spans to — it's overkill for a demo with nowhere to export to.
16. Use injected `TimeProvider` (not `DateTime.Now`/`UtcNow` directly) for any "now" that drives logic a test needs to control (expiry, scheduling, timeout checks) — a test can swap in `FakeTimeProvider` to fix the instant, which a static `DateTime` call can't be made to do; a one-off timestamp nobody asserts on (a log line, an audit field) can stay a plain `DateTime.UtcNow` call.
...
cd Presentation/WebApp && npm install && npm run dev   # React client (Vite dev server, http://localhost:5173)
```

Target framework: `net10.0` across all .NET projects. Nullable reference types and implicit usings are enabled everywhere. `Presentation/WebApp` is a separate Node/Vite project, not part of `dotNet-arch-template.slnx`.
...
Presentation/WebApp (React, separate Node project)  →  calls API over HTTP
```
...
- **`Presentation/API/`** and **`Presentation/MinimalAPI/`** — two parallel presentation layers deliberately kept side by side (controller-based vs Minimal API), both solving the same use case, both referencing `Application` + `Infrastructure`. Each owns its **own** `Dtos/` (`OrderDto`, `ItemDto`, `QuoteDto`) and its **own** `Mapper/` (`OrderMapper`, `ItemMapper`, `QuoteMapper` — static extension classes, one per type, `ToDomain()`/`ToDto()`) — DTOs are a presentation-only concept and never appear below this layer, and the two presentation projects' DTOs/mappers are independent copies, not shared, by design. `Program.cs` in both wires DI via `AddApplicationServices()`/`AddInfrastructureServices()`. `API` exposes the use case via `Controllers/OrdersController.cs` (`POST api/Orders/Quote`); `MinimalAPI` exposes it via `Endpoints/OrderEndpoints.cs` (`MapOrderEndpoints()` extension, same one-extension-per-concern pattern as Rule 5, registered in `Program.cs`; route `POST /orders/quote`), returning `Task<Results<Ok<QuoteDto>, BadRequest<string>>>` via `TypedResults` rather than a bare `IResult`, so each outcome self-describes its status code for the OpenAPI generator instead of relying on convention. Both presentation projects wire native OpenAPI (`AddOpenApi()`/`MapOpenApi()`), Swashbuckle (`AddSwaggerGen()`/`UseSwagger()`/`UseSwaggerUI()`), and Scalar (`MapScalarApiReference()`) side by side in `Program.cs`, all dev-only (`IsDevelopment()`) — deliberately redundant for learning/comparison in this repo, not a pattern to carry into a real project (pick one). `MinimalAPI` additionally needs `AddEndpointsApiExplorer()` before `AddSwaggerGen()`, since — unlike `API` — it has no `AddControllers()` call to register that for it, and without it Swashbuckle can't discover the minimal API endpoints. Both now also wire the Rule 8 global handler: `Middleware/GlobalExceptionHandler.cs` (`IExceptionHandler`, one copy per project like the DTOs/mappers) logs the exception, then writes a generic RFC 7807 `ProblemDetails` (via `IProblemDetailsService`) so the real exception detail never reaches the caller. Registered in `Program.cs` via `AddExceptionHandler<GlobalExceptionHandler>()` + `AddProblemDetails()` (adds a `traceId` extension) + `UseExceptionHandler(...)`.

  Both also now wire **logging** (`Middleware/RequestLoggingMiddleware.cs`, one copy per project): pushes a trace ID + correlation ID into `ILogger.BeginScope` per Rule 14, logging Info at request start/end. Rather than always minting a fresh pair, it first checks the incoming request for `X-Trace-Id`/`X-Correlation-Id` headers and reuses those when present — so a request forwarded from another service continues that service's trace instead of starting a new one; falls back to `HttpContext.TraceIdentifier` + a fresh `Guid.NewGuid()` otherwise. The resolved `CorrelationId` is also stashed on `HttpContext.Items` so a controller further down the pipeline can forward it itself (see `OrdersController` below). `API` uses **Serilog** (Console + rolling file, configured in code, with an explicit `outputTemplate` including `{Properties}` — Serilog's default template drops `BeginScope` values entirely) — `MinimalAPI` uses **NLog** (Console + file, configured via `nlog.config`, whose file target path is prefixed with `${currentdir}` since NLog otherwise resolves relative paths against the bin output folder on .NET Core, not the working directory) — deliberately different libraries for side-by-side comparison, same spirit as the OpenAPI/Swagger/Scalar redundancy.

  **Cross-service trace demo**: `OrdersController.Quote` (API), after a successful `PrepareQuote`, calls MinimalAPI's `Endpoints/LoadLogsForTestEndpoint.cs` (`GET /diagnostics/load-logs-for-test`, mapped via `MapLoadLogsForTestEndpoint()`) over a named `HttpClient` ("MinimalApi", registered in `Program.cs`, base address from `Services:MinimalApiBaseUrl` in `appsettings.json`), forwarding `HttpContext.TraceIdentifier` and the stashed `CorrelationId` as `X-Trace-Id`/`X-Correlation-Id` headers — treating the two presentation projects as if they were separate services. The endpoint itself just logs from its lambda, no Application-layer service (unlike the quote use case) — a manual-verification tool only, not unit tested; it never reads the headers directly, since MinimalAPI's own `RequestLoggingMiddleware` already continues the trace from them (see above), so this call's log lines all land under the same `TraceId`/`CorrelationId` as the original `/api/Orders/Quote` request. This replaces the earlier `ILogTestService`/`LogTestService` (Application-layer) + `DiagnosticsController`/`DiagnosticsEndpoints` (`GET .../LogTest?fail=`) demo, which only proved the scope reached Application — it did not demonstrate cross-service propagation and has been deleted.

  **JWT auth**: `Auth/JwtTokenGenerator.cs` (per project) signs a JWT from a fixed demo username/password + signing key in `appsettings.json`'s `Auth`/`Jwt` sections — no real user store. `Controllers/AuthController.cs` (API, `POST api/Auth/token`) / `Endpoints/AuthEndpoints.cs` (MinimalAPI, `POST /auth/token`) issue the token; the quote endpoint requires it (`[Authorize]` / `.RequireAuthorization()`). `Program.cs` wires `AddAuthentication().AddJwtBearer(...)` validating against the same config.

  **Rate limiting**: built-in `Microsoft.AspNetCore.RateLimiting`, Fixed Window, 3 requests/30s. `RateLimiting/RateLimitPartitionKeyResolver.cs` (per project) is a small static helper — not an inline lambda — so the "key by authenticated user, else by IP" logic is unit-testable; the quote endpoint uses the `"PerUser"` policy, the token endpoint uses `"PerIp"` (no identity exists yet at that point).

  **CORS** (`API` only): a `"ReactDev"` policy allows `http://localhost:5173` (the Vite dev server) — needed only because `Presentation/WebApp` calls `API` from a different origin.

  **TimeProvider demo** (`API` only): `OrdersController.Quote` calls a private `DemoTimeProviderVsDateTimeNow()` after preparing a quote, logging `DateTime.UtcNow` next to an injected `TimeProvider`'s `GetUtcNow()` side by side — per Rule 16, illustrating that a static `DateTime` call can't be controlled from a test while the injected `TimeProvider` can. `TimeProvider.System` is registered as a singleton in `Program.cs`; `OrdersControllerTests` swaps in `FakeTimeProvider` (`Microsoft.Extensions.TimeProvider.Testing`) fixed to a known instant and asserts the logged structured value matches it.
- **`Presentation/WebApp/`** — a plain React + Vite client (JavaScript, no TypeScript/state library), not part of the .NET solution. `src/App.jsx` composes two components: `LoginForm.jsx` (`POST /api/Auth/token`, storing `{ token, expiresAtUtc }` in `sessionStorage`) and `QuoteForm.jsx`, whose Quote button is disabled unless a stored token's `exp` claim (decoded client-side, no library) is still in the future; calls `POST /api/Orders/quote` with the bearer token and shows a friendly message on `429`/`401`. Targets `API` specifically (arbitrary choice — `MinimalAPI` would work identically), via `VITE_API_BASE_URL` read from `.env` (gitignored; `.env.example` is the committed template) rather than hardcoded. `vite.config.js` pins the dev server to port `5173` (`strictPort: true`), matching `API`'s CORS policy. No automated tests — verified manually in a browser.
- **`Tests/`** — one xUnit project, organized into per-layer folders rather than one test project per layer: `ApplicationTests/Services` (`QuoteServiceTests`), `ApplicationTests/Results` (`ResultTests`), `APITests/Mapper` (mirroring `Presentation/API/Mapper/`), `APITests/Controllers` (`OrdersControllerTests`, `AuthControllerTests`, via `Moq`), `APITests/Middleware` and `MinimalAPITests/Middleware` (`GlobalExceptionHandlerTests`, `RequestLoggingMiddlewareTests`, one per project, via `Moq`), `APITests/Auth` / `MinimalAPITests/Auth` (`JwtTokenGeneratorTests` — decodes the generated token and checks its claims/expiry), `APITests/RateLimiting` / `MinimalAPITests/RateLimiting` (`RateLimitPartitionKeyResolverTests`), `MinimalAPITests/Endpoints` (`AuthEndpointsTests` — MinimalAPI's auth logic is pulled into a named static method, `AuthEndpoints.HandleToken`, specifically so it's testable the same way a controller action is), `DomainTests/Models` + `DomainTests/Services` (entities and discount strategies). The cross-service trace demo endpoint (`LoadLogsForTestEndpoint`) and the rate limiter's actual throttling behavior are deliberately **not** unit tested — the former is a manual-verification tool, the latter is framework behavior with no `WebApplicationFactory`/integration-test precedent in this repo. `OrdersControllerTests` mocks `IHttpClientFactory` (returning an `HttpClient` over a fake `HttpMessageHandler`) so its now-outbound call to MinimalAPI never hits the network, and sets `ControllerContext.HttpContext` on the success case since the controller reads `HttpContext.TraceIdentifier`/`Items["CorrelationId"]`. `Builders/` holds fluent Test Data Builders (one per DTO/domain type, e.g. `OrderBuilder`/`OrderDtoBuilder`, `QuoteBuilder`/`QuoteDtoBuilder`) used to keep test arrange-sections terse — extend this pattern (reused across the tests that need it) rather than constructing objects inline, but only once a builder is actually reused by more than one test; a one-off object used in a single test can stay inline. `Tests.csproj` references `Presentation/API` and `Presentation/MinimalAPI` (needed for `APITests` and `MinimalAPITests`) and adds `Moq` (controller/middleware mocking), `FluentAssertions` (kept as a commented-out alternative assertion next to the live xUnit `Assert`/`Assert.Equivalent` call in each test, for reference, not actively used), and `Microsoft.Extensions.TimeProvider.Testing` (`FakeTimeProvider`, for the `TimeProvider` demo test).
...
- Rule 8's global exception handler is now implemented (`GlobalExceptionHandler` in both `Presentation/API/Middleware` and `Presentation/MinimalAPI/Middleware`, each fully covered by its own `GlobalExceptionHandlerTests`) — previously just described by the rule, not yet built.
- JWT auth is a learning/demo implementation, not production-ready: a single fixed username/password from `appsettings.json` (no user store, no DB), a signing key committed in plain text in `appsettings.json`, a short 2-minute token expiry (chosen so expiry is actually observable while testing), and no revocation mechanism (JWTs are stateless — once issued, a token is valid until it expires, full stop).
- Rate limiting's Fixed Window uses a short 30-second window (3 requests) for the same demo-ability reason as the JWT expiry — not a production-sized limit.
- Serilog (`API`) is configured entirely in code rather than bound from `appsettings.json`, for simplicity; NLog (`MinimalAPI`) uses its own `nlog.config` file instead, per NLog's normal convention — the two projects deliberately don't share one configuration approach, same spirit as using two different logging libraries in the first place.
